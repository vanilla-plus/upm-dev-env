...
namespace Google.OpenLocationCode
{
    ...
    /// <list type="bullet">
    /// <item><see cref="OpenLocationCodeUtility(string)"/></item>
    /// <item><see cref="OpenLocationCodeUtility(double,double,int)"/></item>
    ...
    /// </code></example>
    /// </summary>
    public sealed class OpenLocationCodeUtility
    {
        ...
        static OpenLocationCodeUtility()
        {
            for (int i = 0,
                     digitVal = 0;
                 i < IndexedDigitValues.Length;
                 i++)
            {
                int digitIndex = CodeAlphabet[digitVal] - IndexedDigitValueOffset;

                IndexedDigitValues[i] = (digitIndex == i) ?
                                            digitVal++ :
                                            -1;
            }
        }
        ...
        /// <summary>
        /// Creates an <see cref="OpenLocationCodeUtility"/> object for the provided full code (or <see cref="CodeDigits"/>).
        ...
        public OpenLocationCodeUtility(string code)
        {
            if (code == null)
            {
        ...
        /// <summary>
        /// Creates an <see cref="OpenLocationCodeUtility"/> object encoded from the provided latitude/longitude coordinates
        ...
        public OpenLocationCodeUtility(double latitude,
                                double longitude,
                                int codeLength = CodePrecisionNormal)
        {
            Code = Encode(latitude,
                          longitude,
                          codeLength);
        ...
        /// <summary>
        /// Creates an <see cref="OpenLocationCodeUtility"/> object encoded from the provided geographic point coordinates
        ...
        public OpenLocationCodeUtility(GeoPoint point,
                                int codeLength = CodePrecisionNormal) :
            this(point.Latitude,
                 point.Longitude,
                 codeLength) { }
        ...
        public ShortCode Shorten(double referenceLatitude,
                                 double referenceLongitude)
        {
            return ShortenValid(Decode(),
                                Code,
                                referenceLatitude,
                                referenceLongitude);
        }
        ...
        {
            return Shorten(referencePoint.Latitude,
        ...
        {
            return this == obj || (obj is OpenLocationCodeUtility olc && olc.Code == Code);
        }
                                    ...
                                    int codeLength = CodePrecisionNormal)
        {
            ...
            codeLength = Math.Min(codeLength,
                                  MaxDigitCount);
            ...
            {
                codeBuilder.Remove(codeLength,
                                   SeparatorPosition - codeLength);
        ...
        public static string Encode(GeoPoint point,
                                    int codeLength = CodePrecisionNormal)
        {
            return Encode(point.Latitude,
                          point.Longitude,
                          codeLength);
        }
        ...
        private static CodeArea DecodeValid(string codeDigits)
        {
            ...
            int pairPartLength = Math.Min(codeDigits.Length,
                                          PairCodeLength);

            int codeLength = Math.Min(codeDigits.Length,
                                      MaxDigitCount);
            ...
            return new CodeArea((double) latVal                 / LatIntegerMultiplier,
        ...
        public static ShortCode Shorten(string code,
                                        ...
                                        double referenceLongitude)
        {
            ...
            return ShortenValid(Decode(code),
                                code,
                                referenceLatitude,
                                referenceLongitude);
        }
        ...
        private static ShortCode ShortenValid(CodeArea codeArea,
                                              string code,
                                              double referenceLatitude,
                                              double referenceLongitude)
        {
            ...
            double range = Math.Max(Math.Abs(referenceLatitude  - center.Latitude),
                 ...
                 i--)
            {
                ...
                {
                    // We're done.
                    return new ShortCode(code.Substring(i * 2),
                                         valid: true);
                }
            }
        ...
        {
            return Math.Min(Math.Max(latitude,
                                     -LatitudeMax),
                            LatitudeMax);
        }
        ...
        private static string NormalizeCode(string code)
        {
            ...
            {
                return code +
                       new string(PaddingCharacter,
                                  SeparatorPosition - code.Length) +
                       SeparatorCharacter;
            }
        ...
        internal static string TrimCode(string code)
        {
            ...
            return codeBuilder.Length != code.Length ?
        ...
        public class ShortCode
        {
            ...
            /// Use <see cref="OpenLocationCodeUtility(string)"/> for full codes.
            /// </summary>
            ...
            internal ShortCode(string shortCode,
                               bool valid)
            {
                Code = shortCode;
            }
            ...
            /// <param name="referenceLatitude">The reference latitude in decimal degrees.</param>
            /// <param name="referenceLongitude">The reference longitude in decimal degrees.</param>
            public OpenLocationCodeUtility RecoverNearest(double referenceLatitude,
                                                   double referenceLongitude)
            {
                return RecoverNearestValid(Code,
                                           referenceLatitude,
                                           referenceLongitude);
            }
            ...
            public static OpenLocationCodeUtility RecoverNearest(string shortCode,
                                                          double referenceLatitude,
                                                          double referenceLongitude)
            {
                string validCode = ValidateCode(shortCode);

                if (IsCodeFull(validCode)) return new OpenLocationCodeUtility(validCode);

                return RecoverNearestValid(ValidateShortCode(validCode),
                                           referenceLatitude,
                                           referenceLongitude);
            }


            private static OpenLocationCodeUtility RecoverNearestValid(string shortCode,
                                                                double referenceLatitude,
                                                                double referenceLongitude)
            {
                ...
                double prefixPrecision = Math.Pow(EncodingBase,
                                                  2 - (digitsToRecover / 2));

                // Use the reference location to generate the prefix.
                string recoveredPrefix =
                    new OpenLocationCodeUtility(referenceLatitude,
                                         referenceLongitude).Code.Substring(0,
                                                                            digitsToRecover);

                // Combine the prefix with the short code and decode it.
                OpenLocationCodeUtility recovered               = new OpenLocationCodeUtility(recoveredPrefix + shortCode);
                ...
                if (latitudeDiff                        > prefixPrecision / 2 &&
                    recoveredLatitude - prefixPrecision > -LatitudeMax)
                {
                ...
                return new OpenLocationCodeUtility(recoveredLatitude,
                                            recoveredLongitude,
                                            recovered.CodeDigits.Length);
            }
